...
4. Navigation:
- Move window one monitor down **Shift+Ctrl+Super+J**
- Move window one monitor left **Shift+Ctrl+Super+H**
- Move window one monitor right **Shift+Ctrl+Super+L**
- Move window one monitor up **Shift+Super+Ctrl+K**
...
```bash
sudo pacman -S libgphoto2 gphoto2 gvfs-gphoto2 geeqie dcraw
sudo vim /usr/share/thumbnailers/gdk-pixbuf-thumbnailer.thumbnailer
```
Add this line to MimeType=
```python
image/x-xpixmap;image/x-canon-cr2;
```
